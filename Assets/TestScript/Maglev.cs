...
public class Maglev : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Vector3 centerOfMass = transform.TransformPoint(_rb.centerOfMass);
        if (Input.GetKeyDown(KeyCode.U) && _curCD == 0)
        {
            _curHeight = transform.position.y + _maxHeight;
            _t = 4;
        }
    ...
    //[SerializeField] AnimationCurve motorTorque = new AnimationCurve(new Keyframe(-10, 0), new Keyframe(0, 0), new Keyframe(10, 0));
    /// <summary>
        ...
        if (_curDuration)
        {
            ...
            foreach (Vector3 p in _pos)
            {
                ...
                if (Physics.Raycast(_ray, out _raycastHit, 100))
                {
                    ...
                    //float addCoe = useForce * motorTorque.Evaluate();
