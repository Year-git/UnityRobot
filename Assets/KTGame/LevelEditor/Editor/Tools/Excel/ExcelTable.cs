...
{
    public Dictionary<int, ExcelTableCell> data;

    public virtual ExcelTableCell this[int index]
    {
        get
        {
            return data[index];
    ...
    public virtual Dictionary<int, ExcelTableCell> Columns
    {
        get
        {
            return data;
...
public class ExcelTable
{
    ...
    public ExcelTable(ExcelWorksheet sheet)
    {
        ...
        for (int row = 1; row <= NumberOfRows; row++)
        {
            ...
            {
                string value = sheet.Cells[row, column].Value == null ? "" : sheet.Cells[row, column].Value.ToString();
    ...
    {
        Dictionary<int, ExcelTableCell> newRow = new Dictionary<int, ExcelTableCell>();

        for(int i=0;i<NumberOfColumns;i++)
        {
            newRow.Add(i + 1, new ExcelTableCell(NumberOfRows + 1, i + 1, ""));
        }

        return new ExcelRow() { data=newRow };
    }

    public void AddRow(ExcelRow row)
    {
        cells.Add(NumberOfRows + 1, row.data);
        NumberOfRows++;
    }

    public void DeleteRow(ExcelRow row)
    {
        if(cells.ContainsKey(row.RowIndex))
        {
            cells.Remove(row.RowIndex);
            NumberOfRows--;
        }
    }
