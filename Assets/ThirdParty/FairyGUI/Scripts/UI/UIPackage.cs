...
namespace FairyGUI
{
	...
	public class UIPackage
	{
        ...
        string packageName = descFilePath.Replace("Assets/Res/FGUI/", "");
		string fui = ("fgui_" + packageName + "_fui").ToLower();
		string res = ("fgui_" + packageName + "_res").ToLower();
		string path = Framework.AssetBundleLoader._wwwPath.Replace("file://", "");
		string url_fui = $"{path}/{fui}";
		string url_res = $"{path}/{res}";
		Debug.Log(url_fui);
		AssetBundle assetBundle_fui = AssetBundle.LoadFromFile(url_fui);
		AssetBundle assetBundle_res = AssetBundle.LoadFromFile(url_res);
		return AddPackage(assetBundle_fui, assetBundle_res);
#else
